...
import PobololomolonoSetup from './PobololomolonoSetup'
...
const Pobololomolono = () => {
    ...
    const createMatch = async () => {
        const { matchID } = await lobbyClient.createMatch("Pobololomolono", {
            numPlayers: 6,
        })
        // Changes URL to end with /match/matchID
        window.location.href = `/pobololomolono/match/${matchID}`;
    }
    ...
    const joinMatch = async () => {
        try {
            window.location.href = `/pobololomolono/match/${joinMatchID}`;
            setJoinMatchID("");
        } catch {
            setShowError(true)
        }
    ...
    return (
        <div>
            ...
            <BrowserRouter>
                <Switch>
                    <Route
                        path="/pobololomolono/match/:matchID"
                        render={(props) => {
                            const { matchID } = props.match.params;
                            return (
                                <>
                                    <PobololomolonoSetup {...{ matchID, lobbyClient, server }}/>
                                </>
                            );
                        }}
                    />
                    <Route path="/pobololomolono"
                        render={(props) => {
                            return (

                                <div className={"main-menu"}>
                                    <h1 className={"main-menu-header"}>Welcome to Pobololomolono</h1>
